...
const char *websocket_server_host = "10.236.80.50";
...
void micTask(void *parameter) {
  ...
  while (1) {
    ...
    esp_err_t result = i2s_read(I2S_PORT, &sBuffer, bufferLen, &bytesIn,
                                pdMS_TO_TICKS(100) // ✅ Safe timeout
    );

    if (result == ESP_OK && bytesIn > 0 && isWebSocketConnected) {

      if (client.available()) { // ✅ Prevent socket block
        client.sendBinary((const char *)sBuffer, bytesIn);
      }

    } else if (!isWebSocketConnected) {
      Serial.println("WS disconnected, reconnecting...");
      connectWSServer(); // ✅ Auto reconnect
    }
    vTaskDelay(1); // ✅ Yield to WiFi + TCP + DMA
  }
}
